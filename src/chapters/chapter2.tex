...
\begin{itemize}
    ...
    \item Algolia APIs: These are used to create an index of all skill items in the \glsxtrshort{icd} toolset and provide search capabilities in the application.
...
\newpage
{\color{purple}Algolia} was used to enhance the application's search capabilities. Algolia is a powerful and flexible search and discovery API that delivers real-time results from the first keystroke. Its powerful features enabled the application to handle the extensive data from the \glsxtrshort{icd} toolset and provide a user-friendly experience.
...
The project's primary goal was to create a system that allows {\color{newBlue}\textbf{HR}} managers to effectively define job roles using the \glsxtrfull{icd}. The \glsxtrshort{icd}, a comprehensive toolset from Japan, includes a {\color{darkPurple}\textbf{Task Dictionary}} and a {\color{darkgreen}\textbf{Skill Dictionary}}. Originally in Excel format, the \glsxtrshort{icd} integrates elements from three previous Japanese skill standards, making it a valuable resource for defining IT roles.
