...
The \glsxtrshort{icd} toolset web application will cater to a broad range of actors, each with their own specific needs and expectations. These actors can be grouped into three main categories: Companies/Organizations, Individuals, and Educational Institutions.
...
%     \item Interoperability: The system should seamlessly integrate with the \glsxtrshort{icd} toolset and the Airtable database. It should also provide APIs or other means for possible future integrations with other systems.
% \end{enumerate}
...
% \item Maintainability: The system should be modular for easy updates, modifications, and debugging. It should also have tools for issue tracking and resolution.
\item Interoperability: The system should integrate with the \glsxtrshort{icd} toolset and the Airtable database, providing APIs for potential future system integrations.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth]{\includegraphics[width=1.25\linewidth]{src/assets/images/Sprints.jpg}}
...
In this section, we will present the interface mockups for our project. These mockups provide a visual representation of the user interface (UI) that we have designed for our application. They serve as a blueprint for the layout and functionality of the application, illustrating how users, particularly HR managers, will interact with the system. The interface mockups were created with a focus on user-friendliness and intuitive navigation, ensuring that HR managers can easily utilize the \glsxtrfull{icd} toolset through our application. From account creation and management to job description definition and advanced features, these mockups encapsulate the entire user journey within our application.
...
% \newpage
FlutterFlow also supports custom code in {\color{purple} Dart} \cite{dart}, which is a client-optimized language for fast apps on any platform. Dart was used extensively in this project to implement custom widgets, functions, and actions to meet the specific needs of the application. The language's robust library and powerful features facilitated the development of interactive features in the application, such as dropdowns for job descriptions, the Algolia search function for skill selection, and the chatbot for user assistance.
...
Express.js is a fast, unopinionated, and minimalist web framework for Node.js. It's designed for building web applications and APIs. In this project, Express.js was used to structure the data from the \glsxtrshort{icd} toolset and handle API routes, ensuring efficient and reliable data management.
...
{\color{purple}Algolia} \cite{algolia} was used to enhance the application's search capabilities. Algolia is a powerful and flexible search and discovery API that delivers real-time results from the first keystroke. Its powerful features enabled the application to handle the extensive data from the \glsxtrshort{icd} toolset and provide a user-friendly experience.
...
% \newpage
% \subsection{Diagrams and Graphs Generation}
% For the creation of various diagrams and graphs, {\color{purple}Figma} \cite{figma} was extensively utilized. Figma is a cloud-based design tool that allows for collaborative work among team members. It offers a wide range of features that make it an excellent tool for creating intricate diagrams and graphs. These visuals were crucial in representing complex data and workflows in a more understandable and accessible manner.
% \begin{figure}[H]
%     \centering
%     \makebox[\textwidth]{\includegraphics[width=0.4\linewidth]{src/assets/logos/1024px-Figma-logo.svg.png}}
%     \caption{  Logo Of Figma }
%     \label{fig: Figma_Logo}
% \end{figure}

\subsection{Report Building}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth]{\includegraphics[width=0.4\linewidth]{src/assets/logos/LaTeX_logo.svg.png}}
    \caption{  Logo Of Latex }
    \label{fig: Latex_Logo}
\end{figure}

With these technologies combined, We were able to build an efficient and user-friendly application that simplifies the use of the \glsxtrshort{icd} toolset.
...
Our web application simplifies the \glsxtrshort{icd} toolset, originally in Excel files and graphs, by making the data user-friendly. The MVP focuses on Human Resource Managers, enabling them to define job descriptions. Users can select jobs from dropdown menus, view required skills, and use the Algolia search engine for skill filtering. The application shows tasks related to selected skills and generates job descriptions via the GPT-3.5 Turbo model from OpenAI. Generated descriptions are stored in a Firestore database for updating or deletion. A GPT-API-powered chatbot is included for context-specific \glsxtrshort{icd} toolset information.


\subsection{General Architecture}

The application's architecture is designed with a focus on simplifying the interaction between the user and the \glsxtrshort{icd} toolset data. It consists of several key components:
...
\begin{itemize}
    ...
    \item Back-End: Leveraging JavaScript and Express.js, our back-end structures the raw \glsxtrshort{icd} data and exposes it via APIs hosted on Render.
    \item Database: The application uses two databases: Airtable and Firestore. The \glsxtrshort{icd} toolset data is imported and filtered into an Airtable database, while Firestore is used to store user information and generated job descriptions.
    ...
    \item Chatbot: A chatbot is included in the application to provide users with context-specific information about the \glsxtrshort{icd} toolset. The chatbot also uses the GPT-API from OpenAI and is designed to provide suggestions that are updated based on the user's interactions with the application.
\end{itemize}
...
The project's primary goal was to create a system that allows {\color{newBlue}\textbf{HR managers}} to effectively define job roles using the \glsxtrfull{icd}. The \glsxtrshort{icd}, a comprehensive toolset from Japan, includes a {\color{darkPurple}\textbf{Task Dictionary}} and a {\color{darkPurple}\textbf{Skill Dictionary}}. Originally in Excel format, the \glsxtrshort{icd} integrates elements from three previous Japanese skill standards, making it a valuable resource for defining IT roles.


\begin{itemize}
    \item Task 1: \\
          Import the raw \glsxtrshort{icd} toolset data into the Airtable database with necessary modifications.
    \item Task 2: \\
...
To effectively utilize the \glsxtrshort{icd} toolset, the first step was to import the raw data into {\color{limeGreen}\textbf{Airtable}}, a cloud-based database management system. 
This required converting the Excel-based \glsxtrshort{icd} toolset into a format compatible with Airtable. 
...
With the data pipeline fully established, attention was turned to developing features within the application that allowed users to interact with the \glsxtrshort{icd} data. 
...
\subsection{\glsxtrshort{icd} Project Management Canva and Mindmap}
...
understanding of the complex structure of the \glsxtrshort{icd} toolset and its nested structure. 
